  ...
  <div>
    <div class="table-wrap">
      <table class="data-table">
        <thead>
          <tr><th>图书</th><th>作者</th><th>借阅日期</th><th>应还日期</th><th>状态</th><th>续借次数</th><th>操作</th></tr>
        </thead>
        <tbody>
          ...
          <tr v-for="r in records" :key="r.id">
            ...
            <td>
              <div class="action-btns">
                <button class="btn-sm" title="详情" @click="$emit('view', r)">📄</button>
                <button v-if="isAdmin && r.status !== 'returned'" class="btn-sm btn-sm-ok" title="还书" @click="$emit('return', r)">📥</button>
                <button v-if="isAdmin && r.status !== 'returned'" class="btn-sm btn-sm-warn" title="续借" @click="$emit('renew', r)">🔄</button>
              </div>
            </td>
          </tr>
        </tbody>
...
const props = defineProps<{ records: BorrowRecord[]; loading: boolean; total: number; current: number; pageSize: number; isAdmin: boolean }>();
const emit = defineEmits<{ (e: 'view', r: BorrowRecord): void; (e: 'return', r: BorrowRecord): void; (e: 'renew', r: BorrowRecord): void; (e: 'page-change', p: number): void }>();
...
function statusClass(s: string) { return s === 'borrowed' ? 'tag-blue' : s === 'returned' ? 'tag-green' : 'tag-red'; }
function statusLabel(s: string) { return s === 'borrowed' ? '借阅中' : s === 'returned' ? '已归还' : '逾期'; }
</script>
